pipeline {
    ...
    triggers {
        upstream(
            upstreamProjects: 'open-simulation-platform/cse-core/v0.5.0, open-simulation-platform/cse-client/v0.5.0',
                ...
                stage('Windows') {
                    ...
                    stages {
                        stage ('Get dependencies') {
                            steps {
                                copyArtifacts(
                                    projectName: 'open-simulation-platform/cse-client/v0.5.0',
                        ...
                        stage ('Prepare dist') {
                            steps {
                                dir ('src/cse-server-go/dist/bin') {
                                    ...
                                    sh 'curl https://github.com/NTNU-IHB/FMU-proxy/releases/download/v0.5.3/fmu-proxy.jar -o fmu-proxy.jar -L'
                                }
                                dir ('src/cse-server-go/dist') {
                                    sh 'cp -rf ../run-windows.cmd .'
                                    sh 'cp -rf ../run-fmuproxy.cmd .'
                                }
                            }
                        }
                        stage ('Zip dist') {
